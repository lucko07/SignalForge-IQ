...
function loadServiceAccountFromBase64() {
  if (!serviceAccountBase64.trim()) {
    throw new Error(
      "Missing Firebase Admin credentials. Set FIREBASE_SERVICE_ACCOUNT_JSON_BASE64 in Vercel."
    );
  }
  ...
  try {
    ...
    throw new Error(
      `FIREBASE_SERVICE_ACCOUNT_JSON_BASE64 could not be base64-decoded: ${getErrorMessage(error)}`
    );
  }
  ...
  try {
    ...
    throw new Error(
      `FIREBASE_SERVICE_ACCOUNT_JSON_BASE64 did not decode to valid JSON: ${getErrorMessage(error)}`
    );
  }

  if (!isValidServiceAccount(serviceAccount)) {
    throw new Error(
      "FIREBASE_SERVICE_ACCOUNT_JSON_BASE64 is missing required service account fields: project_id, client_email, and private_key."
    );
  }
